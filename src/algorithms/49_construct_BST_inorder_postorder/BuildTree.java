class Solution {
	...
	public TreeNode buildTree(int[] inorder, int[] postorder) {
		...
		return helper(inorder, postorder, postorder.length-1, 0, 0, inorder.length-1, map);
	}

	private TreeNode helper(int[] inorder, int[] postorder, int postStart, int postEnd, int inStart, int inEnd, HashMap<Integer, Integer> map){
		...
		root.left = helper(inorder, postorder, postStart-(inEnd-rootIndex)-1, postEnd, inStart, rootIndex-1, map );
		root.right = helper(inorder, postorder, postStart-1, postStart-(inEnd-rootIndex), rootIndex+1, inEnd, map);
